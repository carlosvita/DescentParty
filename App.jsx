...
const CLASS_SKILLS = {
  ...
  "Cavaleiro": {
    skills: [
      ...
      { name: "Treino de Defesa", xp: 2, stamina: null, dice: "🟫", rules: "Com Escudo, +1 dado marrom na defesa. Pode exaurir esta carta em vez de exaurir o Escudo." },
  ...
  "Elementalista": {
    skills: [
      ...
      { name: "Poder Vulcânico", xp: 2, stamina: null, noExhaust: true, rules: "Enquanto Garra exaurida, +1 Escudo na defesa. Enquanto Labareda exaurida, após monstro atacar você, ele sofre 1 HP." },
      { name: "Abraço da Natureza", xp: 3, stamina: null, noExhaust: true, rules: "+1 💧. Ao usar Maré, +1 monstro alvo. Ao usar Garra, atinja figuras não-tenentes igual ao nº de cartas 🌀 exauridas." },
...
// ─── DADOS DO JOGO ──────────────────────────────────────────────────────────
// Dados de ataque: azul, vermelho, amarelo, verde
// Dados de defesa: marrom, cinza, preto
// Cada face: { range, hearts, surge, shield, miss }

const DICE = {
  blue: {
    name: "Azul", type: "attack", color: "#4a90d9", emoji: "🟦",
    faces: [
      { label: "❌", miss: true },
      { label: "2, ⚡", range: 2, surge: 1 },
      { label: "3, ❤️❤️", range: 3, hearts: 2 },
      { label: "4, ❤️❤️", range: 4, hearts: 2 },
      { label: "5, ❤️", range: 5, hearts: 1 },
      { label: "6, ⚡", range: 6, surge: 1 },
    ]
  },
  red: {
    name: "Vermelho", type: "attack", color: "#d94a4a", emoji: "🟥",
    faces: [
      { label: "❤️", hearts: 1 },
      { label: "❤️❤️", hearts: 2 },
      { label: "❤️❤️", hearts: 2 },
      { label: "❤️❤️", hearts: 2 },
      { label: "❤️❤️❤️", hearts: 3 },
      { label: "❤️❤️❤️, ⚡", hearts: 3, surge: 1 },
    ]
  },
  yellow: {
    name: "Amarelo", type: "attack", color: "#d9c84a", emoji: "🟨",
    faces: [
      { label: "1, ⚡", range: 1, surge: 1 },
      { label: "1, ❤️", range: 1, hearts: 1 },
      { label: "2, ❤️", range: 2, hearts: 1 },
      { label: "❤️, ⚡", hearts: 1, surge: 1 },
      { label: "❤️❤️", hearts: 2 },
      { label: "❤️❤️, ⚡", hearts: 2, surge: 1 },
    ]
  },
  green: {
    name: "Verde", type: "attack", color: "#4ad97a", emoji: "🟩",
    faces: [
      { label: "❤️", hearts: 1 },
      { label: "⚡", surge: 1 },
      { label: "1, ⚡", range: 1, surge: 1 },
  ...
  brown: {
    name: "Marrom", type: "defense", color: "#a3845c", emoji: "🟫",
    faces: [
      { label: "—", shields: 0 },
      { label: "—", shields: 0 },
      { label: "—", shields: 0 },
  ...
  gray: {
    name: "Cinza", type: "defense", color: "#9ca3af", emoji: "🔲",
    faces: [
      { label: "—", shields: 0 },
      { label: "🛡️", shields: 1 },
      { label: "🛡️", shields: 1 },
      { label: "🛡️", shields: 1 },
      { label: "🛡️🛡️", shields: 2 },
      { label: "🛡️🛡️🛡️", shields: 3 },
    ]
  },
  black: {
    name: "Preto", type: "defense", color: "#4a4a4a", emoji: "⬛",
    faces: [
      { label: "—", shields: 0 },
      { label: "🛡️🛡️", shields: 2 },
      { label: "🛡️🛡️", shields: 2 },
...
const DICE_EMOJI_MAP = { "🟦": "blue", "🟥": "red", "🟨": "yellow", "🟩": "green" };
const DEF_DICE_EMOJI_MAP = { "🟫": "brown", "🔲": "gray", "⬛": "black" };
function parseDiceEmojis(diceStr) {
  if (!diceStr || diceStr === "—") return [];
  return [...diceStr].filter(ch => DICE_EMOJI_MAP[ch]).map(ch => DICE_EMOJI_MAP[ch]);
}
function parseDefDiceEmojis(diceStr) {
  if (!diceStr || diceStr === "—") return [];
  return [...diceStr].filter(ch => DEF_DICE_EMOJI_MAP[ch]).map(ch => DEF_DICE_EMOJI_MAP[ch]);
}

function StatBadge({ label, value, color, theme, onClick }) {
  return (
...
function SkillCard({ skill, selected, onToggle, disabled, theme, exhausted, onExhaustToggle, onDiceRoll }) {
  const isInit = skill.xp === 0;
  const isLight = theme === "light";
  const hasExhaust = !skill.noExhaust && /exaur|exaust/i.test(skill.rules);
  return (
    <div
    ...
    >
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 6 }}>
        ...
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          {skill.dice && selected && onDiceRoll && (
            <span onClick={e => { e.stopPropagation(); onDiceRoll(skill.name, skill.dice); }} style={{ fontSize: 11, background: "rgba(100,180,255,0.12)", color: "#7cb8f0", padding: "2px 7px", borderRadius: 6, cursor: "pointer" }}>{skill.dice}</span>
          )}
...
export default function RPGCharacterSheet() {
  ...
  const [attrTest, setAttrTest] = useState(null);
  const [diceLog, setDiceLog] = useState([]);
  const [showLog, setShowLog] = useState(false);
  const [defRoll, setDefRoll] = useState(null);
  ...
  function rollAttributeTest(attrName, attrValue) {
    const grayFace = DICE.gray.faces[Math.floor(Math.random() * 6)];
    const blackFace = DICE.black.faces[Math.floor(Math.random() * 6)];
    const totalShields = (grayFace.shields || 0) + (blackFace.shields || 0);
    const success = totalShields <= attrValue;
    const result = { attrName, attrValue, grayFace, blackFace, totalShields, success, time: new Date().toLocaleTimeString() };
    setAttrTest(result);
    setDiceLog(prev => [result, ...prev].slice(0, 5));
    setTimeout(() => setAttrTest(null), 4000);
  }

  function rollDefense(defType) {
    const diceKey = defType; // "gray", "brown", or "black"
    const face = DICE[diceKey].faces[Math.floor(Math.random() * 6)];
    const result = { type: "defense", diceName: DICE[diceKey].name, diceEmoji: defDice[diceKey], face, shields: face.shields || 0, time: new Date().toLocaleTimeString() };
    setDefRoll(result);
    setDiceLog(prev => [result, ...prev].slice(0, 5));
    setTimeout(() => setDefRoll(null), 3000);
  }

  function rollAttack(itemName, diceStr) {
    const keys = parseDiceEmojis(diceStr);
    if (keys.length === 0) return;
    const rolls = keys.map(k => {
      const face = DICE[k].faces[Math.floor(Math.random() * 6)];
      return { key: k, name: DICE[k].name, emoji: DICE[k].emoji, color: DICE[k].color, face };
    });
    const miss = rolls.some(r => r.face.miss);
    const totalRange = rolls.reduce((s, r) => s + (r.face.range || 0), 0);
    const totalHearts = rolls.reduce((s, r) => s + (r.face.hearts || 0), 0);
    const totalSurge = rolls.reduce((s, r) => s + (r.face.surge || 0), 0);
    const result = { type: "attack", itemName, rolls, miss, totalRange, totalHearts, totalSurge, time: new Date().toLocaleTimeString() };
    setAttackRoll(result);
    setDiceLog(prev => [result, ...prev].slice(0, 5));
    setTimeout(() => setAttackRoll(null), 5000);
  }
  ...
  return (
    <div ref={sheetRef} style={{
      ...
      {attrTest && (
        ...
        }} onClick={() => setAttrTest(null)}>
          <div onClick={e => e.stopPropagation()} style={{
            background: theme === "dark" ? "#1a1816" : "#ffffff", color: t.text,
            border: `2px solid ${attrTest.success ? "#7ec8a0" : "#e87e7e"}`,
            borderRadius: 16, padding: "24px 32px", textAlign: "center", minWidth: 280,
            boxShadow: "0 8px 40px rgba(0,0,0,0.4)", animation: "fadeIn 0.2s ease",
            fontFamily: "'EB Garamond', serif",
          }}>
            <div style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: 2, opacity: 0.5, marginBottom: 8 }}>
              Teste de {attrTest.attrName}
            </div>
            <div style={{ display: "flex", justifyContent: "center", gap: 16, marginBottom: 16 }}>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 11, opacity: 0.4, marginBottom: 4 }}>Cinza</div>
                <div style={{ fontSize: 32 }}>{DICE.gray.emoji}</div>
                <div style={{ fontSize: 16, marginTop: 4 }}>{attrTest.grayFace.label}</div>
              </div>
              <div style={{ textAlign: "center" }}>
                <div style={{ fontSize: 11, opacity: 0.4, marginBottom: 4 }}>Preto</div>
                <div style={{ fontSize: 32 }}>{DICE.black.emoji}</div>
                <div style={{ fontSize: 16, marginTop: 4 }}>{attrTest.blackFace.label}</div>
              </div>
            </div>
            <div style={{ fontSize: 14, opacity: 0.6, marginBottom: 8 }}>
              🛡️ Total: <span style={{ fontWeight: 700, fontSize: 18 }}>{attrTest.totalShields}</span> vs {attrTest.attrName}: <span style={{ fontWeight: 700, fontSize: 18 }}>{attrTest.attrValue}</span>
            </div>
            <div style={{
              fontSize: 22, fontWeight: 700, fontFamily: "'Cinzel', serif",
              color: attrTest.success ? "#7ec8a0" : "#e87e7e",
              marginTop: 8,
            }}>
              {attrTest.success ? "✓ SUCESSO" : "✗ FALHA"}
            </div>
            <div style={{ fontSize: 11, opacity: 0.3, marginTop: 10 }}>clique para fechar</div>
          </div>
        </div>
      )}
      ...
      {defRoll && (
        <div style={{
          position: "fixed", top: 20, left: "50%", transform: "translateX(-50%)", zIndex: 999,
          background: theme === "dark" ? "#1a1816" : "#ffffff", color: t.text,
          border: `2px solid ${defColors[heroData?.def] || "#9ca3af"}`,
          borderRadius: 12, padding: "14px 24px", textAlign: "center", minWidth: 200,
          boxShadow: "0 6px 30px rgba(0,0,0,0.4)", animation: "fadeIn 0.2s ease",
          fontFamily: "'EB Garamond', serif",
        }} onClick={() => setDefRoll(null)}>
          <div style={{ fontSize: 11, textTransform: "uppercase", letterSpacing: 2, opacity: 0.5, marginBottom: 6 }}>Defesa — Dado {defRoll.diceName}</div>
          <div style={{ fontSize: 36, marginBottom: 4 }}>{defRoll.diceEmoji}</div>
          <div style={{ fontSize: 20, fontWeight: 700, fontFamily: "'Cinzel', serif" }}>
            {defRoll.shields > 0 ? "🛡️".repeat(defRoll.shields) : "—"}
          </div>
          <div style={{ fontSize: 13, opacity: 0.5, marginTop: 4 }}>{defRoll.shields} escudo{defRoll.shields !== 1 ? "s" : ""}</div>
        </div>
      )}
      ...
      {attackRoll && (
        <div style={{
          position: "fixed", top: 0, left: 0, right: 0, bottom: 0, zIndex: 1000,
          background: "rgba(0,0,0,0.5)", display: "flex", alignItems: "center", justifyContent: "center",
        }} onClick={() => setAttackRoll(null)}>
          <div onClick={e => e.stopPropagation()} style={{
            background: theme === "dark" ? "#1a1816" : "#ffffff", color: t.text,
            border: `2px solid ${attackRoll.miss ? "#e87e7e" : "#e8c06a"}`,
            borderRadius: 16, padding: "24px 32px", textAlign: "center", minWidth: 300,
            boxShadow: "0 8px 40px rgba(0,0,0,0.4)", animation: "fadeIn 0.2s ease",
            fontFamily: "'EB Garamond', serif",
          }}>
            <div style={{ fontSize: 13, textTransform: "uppercase", letterSpacing: 2, opacity: 0.5, marginBottom: 12 }}>
              {attackRoll.itemName}
            </div>
            {attackRoll.miss ? (
              <div style={{ fontSize: 28, fontWeight: 700, fontFamily: "'Cinzel', serif", color: "#e87e7e", margin: "16px 0" }}>❌ ERRO</div>
            ) : (<>
              <div style={{ display: "flex", justifyContent: "center", gap: 14, marginBottom: 16 }}>
                {attackRoll.rolls.map((r, i) => (
                  <div key={i} style={{ textAlign: "center" }}>
                    <div style={{ fontSize: 10, opacity: 0.4, marginBottom: 3 }}>{r.name}</div>
                    <div style={{ fontSize: 32 }}>{r.emoji}</div>
                    <div style={{ fontSize: 14, marginTop: 4 }}>{r.face.label}</div>
                  </div>
                ))}
              </div>
              <div style={{ display: "flex", justifyContent: "center", gap: 20, fontSize: 15 }}>
                {attackRoll.totalRange > 0 && <span>📏 <strong>{attackRoll.totalRange}</strong></span>}
                {attackRoll.totalHearts > 0 && <span>❤️ <strong>{attackRoll.totalHearts}</strong></span>}
                {attackRoll.totalSurge > 0 && <span>⚡ <strong>{attackRoll.totalSurge}</strong></span>}
              </div>
            </>)}
            <div style={{ fontSize: 11, opacity: 0.3, marginTop: 12 }}>clique para fechar</div>
          </div>
        </div>
      )}
      ...
      {showLog && (
        <div style={{
          ...
          <div onClick={e => e.stopPropagation()} style={{
            ...
            borderRadius: 16, padding: "24px 28px", minWidth: 300, maxWidth: 400,
            boxShadow: "0 8px 40px rgba(0,0,0,0.4)", animation: "fadeIn 0.2s ease",
            fontFamily: "'EB Garamond', serif",
          }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14 }}>
              <span style={{ fontSize: 14, textTransform: "uppercase", letterSpacing: 2, opacity: 0.5 }}>Log de Dados</span>
              <button onClick={() => setShowLog(false)} style={{ background: "none", border: "none", color: t.text, cursor: "pointer", fontSize: 16, opacity: 0.4 }}>✕</button>
            </div>
            {diceLog.length === 0 ? (
              <p style={{ fontSize: 13, opacity: 0.4, fontStyle: "italic", textAlign: "center", margin: "20px 0" }}>Nenhum teste realizado ainda.</p>
            ) : (
              <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                {diceLog.map((entry, i) => (
                  <div key={i} style={{
                    ...
                    borderLeft: entry.type === "defense"
                      ? `3px solid ${defColors[heroData?.def] || "#9ca3af"}`
                      : entry.type === "attack"
                      ? `3px solid ${entry.miss ? "#e87e7e" : "#e8c06a"}`
                      : `3px solid ${entry.success ? "#7ec8a0" : "#e87e7e"}`,
                    borderRadius: 8,
                  }}>
                    <div style={{ flex: 1 }}>
                      {entry.type === "attack" ? (
                        <>
                          <div style={{ fontSize: 13, fontWeight: 600 }}>{entry.itemName}</div>
                          <div style={{ fontSize: 11, opacity: 0.5, marginTop: 2 }}>
                            {entry.miss ? "❌ Erro" : `${entry.rolls.map(r => r.emoji).join("")} → 📏${entry.totalRange} ❤️${entry.totalHearts} ⚡${entry.totalSurge}`}
                          </div>
                        </>
                      ) : entry.type === "defense" ? (
                        <>
                          <div style={{ fontSize: 13, fontWeight: 600 }}>Defesa <span style={{ fontWeight: 400, opacity: 0.5 }}>({entry.diceName})</span></div>
                          <div style={{ fontSize: 11, opacity: 0.5, marginTop: 2 }}>{entry.diceEmoji} {entry.face.label} → 🛡️{entry.shields}</div>
                        </>
                      ) : (
                        <>
                          <div style={{ fontSize: 13, fontWeight: 600 }}>{entry.attrName} <span style={{ fontWeight: 400, opacity: 0.5 }}>({entry.attrValue})</span></div>
                          <div style={{ fontSize: 11, opacity: 0.5, marginTop: 2 }}>🔲 {entry.grayFace.label} + ⬛ {entry.blackFace.label} = 🛡️{entry.totalShields}</div>
                        </>
                      )}
                    </div>
                    <div style={{ textAlign: "right" }}>
                      {entry.type === "attack" ? (
                        <div style={{ fontSize: 12, fontWeight: 700, color: entry.miss ? "#e87e7e" : "#e8c06a" }}>{entry.miss ? "❌" : `❤️${entry.totalHearts}`}</div>
                      ) : entry.type === "defense" ? (
                        <div style={{ fontSize: 13, fontWeight: 700, color: defColors[heroData?.def] || "#9ca3af" }}>🛡️{entry.shields}</div>
                      ) : (
                        <div style={{ fontSize: 13, fontWeight: 700, color: entry.success ? "#7ec8a0" : "#e87e7e" }}>{entry.success ? "✓" : "✗"}</div>
                      )}
                      <div style={{ fontSize: 9, opacity: 0.3 }}>{entry.time}</div>
                    </div>
                  </div>
                ))}
              </div>
      ...
      <div style={{
        ...
        <div style={{ maxWidth: 720, margin: "0 auto", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          ...
          <div className="no-print" style={{ display: "flex", gap: 6, flexWrap: "wrap", justifyContent: "flex-end" }}>
            ...
            <button onClick={() => setShowLog(!showLog)} style={btnStyle} title="Log de dados">🎲 Log</button>
        ...
        {heroData && (
          <div ref={heroCardRef} style={{
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 12 }}>
              ...
              <div onClick={() => rollDefense(heroData.def)} style={{
                display: "flex", alignItems: "center", gap: 5, cursor: "pointer",
                background: defColors[heroData.def] + "22", border: `1px solid ${defColors[heroData.def]}44`,
                borderRadius: 8, padding: "4px 10px", fontSize: 13, transition: "transform 0.15s ease",
              }}>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 16 }}>
              ...
              <StatBadge label="For" value={heroData.str} color={archData.color} theme={theme} onClick={() => rollAttributeTest("Força", heroData.str)} />
              <StatBadge label="Vont" value={heroData.will} color={archData.color} theme={theme} onClick={() => rollAttributeTest("Vontade", heroData.will)} />
              <StatBadge label="Con" value={heroData.know} color={archData.color} theme={theme} onClick={() => rollAttributeTest("Conhecimento", heroData.know)} />
              <StatBadge label="Perc" value={heroData.perc} color={archData.color} theme={theme} onClick={() => rollAttributeTest("Percepção", heroData.perc)} />
            </div>
                    ...
                    <div key={item.id + "-" + i} style={{
                      ...
                      <div style={{ flex: 1 }}>
                        <div style={{ display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                          ...
                          {item.dice && item.dice !== "—" && (() => {
                            const hasAttackDice = parseDiceEmojis(item.dice).length > 0;
                            const defDiceKeys = parseDefDiceEmojis(item.dice);
                            const hasDefDice = defDiceKeys.length > 0;
                            const isClickable = isEq && (hasAttackDice || hasDefDice);
                            const handleDiceClick = isClickable ? (e) => { e.stopPropagation(); if (hasAttackDice) { rollAttack(item.name, item.dice); } else if (hasDefDice) { rollDefense(defDiceKeys[0]); } } : undefined;
                            return <span onClick={handleDiceClick} style={{ fontSize: 10, background: "rgba(100,180,255,0.12)", color: "#7cb8f0", padding: "1px 6px", borderRadius: 4, cursor: isClickable ? "pointer" : "default" }}>{item.dice}</span>;
                          })()}
              ...
              {[...classSkills.skills].sort((a, b) => {
                ...
                return (
                  <SkillCard
                    ...
                    onDiceRoll={(sName, dStr) => { const ak = parseDiceEmojis(dStr); const dk = parseDefDiceEmojis(dStr); if (ak.length > 0) rollAttack(sName, dStr); else if (dk.length > 0) rollDefense(dk[0]); }}
                  />
                );
              })}
            </div>
